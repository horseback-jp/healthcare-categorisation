# Technical Design Document (TDD) — Evergreen Healthcare Intelligent Routing & Pre-Authorisation Agent
...
### Architecture
- **Single Agent:** `evergreen_voice_assistant`. Acts as the first point of contact for customers calling the support line.
- **Persona & Tone:** Friendly, professional, highly empathetic, and efficient Digital Assistant representing a premium healthcare brand.
- **Scope:** Greeting, caller intent capture (pre-authorisation, consultation, claims), dynamic medical specialty prompting (when required for authorisations), entity resolution (mapping synonyms to official Evergreen Healthcare values), and deterministic telephony handoff with SIP UUI data.

### Tools
| Tool Name | Type | Purpose |
|-----------|------|---------|
| `medical_entity_lookup` | Python function | Takes caller utterance and maps it to official Evergreen Healthcare routing entities (`authorisation_type`, `appointment_type`, `medical_specialty`) using an embedded CSV knowledge base |
...
### Routing Logic
- **Phase 1: Greeting:** Welcome caller to Evergreen Healthcare, introduce as digital assistant, and ask how to help.
...
|----------|-------|---------|
| `before_model` | `evergreen_voice_assistant` | Intercepts `_action_trigger` to execute deterministic transfer tool calls; handles silence retry logic |
| `after_model` | `evergreen_voice_assistant` | Prepends professional goodbye / transfer confirmation before ending session |
...
3. Create agent configuration and structured XML instructions (`evergreen_voice_assistant`)
